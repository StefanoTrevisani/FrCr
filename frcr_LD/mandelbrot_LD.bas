...
GUI "Mandelbrot_LD", PIXELS, ICON, 100
...
BEGIN EVENTS
SELECT CASE CBMSG
global x1a as LDOUBLE
global y1a as LDOUBLE
global x2a as LDOUBLE
global y2a as LDOUBLE
...
'SetCoords
      IF CBCTL = ID_MENU35 THEN
        ...
        remove$(strl$(cx1)+","+strl$(cy1)+","+strl$(cx2)+","+strl$(cy2),SPC$))
        if tally(coord_man$,",") = 3 then
            REMOVE SPC$ FROM UCASE$(coord_man$)
            REMOVE TAB$ FROM UCASE$(coord_man$)
        	cx1 = vall(extract$(coord_man$,","))
            tmp$ = remain$(coord_man$,",")
        	cy1 = vall(extract$(tmp$,","))
            tmp$ = remain$(tmp$,",")
        	cx2 = vall(extract$(tmp$,","))
            tmp$ = remain$(tmp$,",")
        	cy2 = vall(tmp$)
...
local r as integer
local i as integer
global dx as LDOUBLE
global dy as LDOUBLE
local x as LDOUBLE
local y as LDOUBLE
local a as LDOUBLE
local b as LDOUBLE
...
'salvo i dati del file
   ...
   fprint fp1,linea$
   linea$ = "X_MINIMO=" + strl$(cx1)
   fprint fp1,linea$

   linea$ = ";Y minimo es: -1.5"
   fprint fp1,linea$
   linea$ = "Y_MINIMO=" + strl$(cy1)
   fprint fp1,linea$

   linea$ = ";X massimo es: 2.0"
   fprint fp1,linea$
   linea$ = "X_MASSIMO=" + strl$(cx2)
   fprint fp1,linea$

   linea$ = ";Y massimo es: 2.0"
   fprint fp1,linea$
   linea$ = "Y_MASSIMO=" + strl$(cy2)
   fprint fp1,linea$
...
SUB Info
    ...
    MsgBox  "Numero Iterazioni -> " + str$(t) + CRLF$ + _
             "X minimo -> " + strl$ (cx1) + CRLF$ + _
             "Y minimo -> " + strl$ (cy1) + CRLF$ + _
             "X massimo -> " + strl$ (cx2) + CRLF$ + _
             "Y massimo -> " + strl$ (cy2) + CRLF$ + _
