...
GUI "Mandelbrot_B", PIXELS, ICON, 100
...
dim qcx1$[2048] as char
dim qcy1$[2048] as char
dim qcx2$[2048] as char
dim qcy2$[2048] as char
...
'N.B. con i buffers delle variabili stringa a 2048chars max 227*30 (=6810bit)
'N.B. con i buffers delle variabili stringa a 8192chars max 1000*30 (=30000bit)
'precisione pari a 100*30bit (=3000bit)
BignumPrecision(100)
...
SELECT CASE CBMSG
local rc as RECT
local coord_man$[2048*4] as char
local tmp$[2048*4] as char
...
'SetCoords
      IF CBCTL = ID_MENU35 THEN
        ...
        if tally(coord_man$,",") = 3 then
        	...
        	cx1 = atoquad(extract$(coord_man$,","))
            tmp$ = remain$(coord_man$,",")
        	cy1 = atoquad(extract$(tmp$,","))
            tmp$ = remain$(tmp$,",")
        	cx2 = atoquad(extract$(tmp$,","))
            tmp$ = remain$(tmp$,",")
        	cy2 = atoquad(tmp$)
...
'subrutine che trasforma una stringa con un numero in formato esponenziale
'nel formato esteso non esponenziale per poter essere convertito dalla funzione
'atoquad() della libreria bignums.
'esempio:
'-.3456E0 deve diventare -.3456
'-.1234E+1 deve diventare -1.234
'-.1234E1 deve diventare -1.234
'-4.578E-1 deve diventare -.4578
'.3456E0 deve diventare .3456
'.1234E+1 deve diventare 1.234
'.1234E1 deve diventare 1.234
'4.578E-1 deve diventare .4578
SUB ascii_esptonorm(ascii_str$ AS STRING)
dim k as integer
dim prima$ as string
dim dopo$ as string
dim cnt as integer

    'maiuscolizzo nel caso vi sia un simbolo esponenziale minuscolo e
    UCASE$(ascii_str$)
        
    IF INSTR(ascii_str$,"E+") THEN
		k = VAL(REMAIN$(ascii_str$,"E+"))
    	ascii_str$ = EXTRACT$(ascii_str$,"E+")   
    	'sposto il punto decimale di k posizioni in avanti fillando eventualmente con zeri
    	prima$ = EXTRACT$(ascii_str$,".")
    	dopo$ = REMAIN$(ascii_str$,".")
        'print "k= ",k
        'print "prima$= ",prima$
        'print "dopo$= ",dopo$
    	IF LEN(dopo$) < k THEN
    		'fillo con zeri dopo$
    		FOR cnt = LEN(dopo$) TO k
    			dopo$ = dopo$ + "0"
    		NEXT cnt
    		ascii_str$ = prima$ + MID$(dopo$,1,k) + "." + REMAIN$(dopo$,MID$(dopo$,1,k))
        ELSEIF LEN(dopo$) = k THEN
        	ascii_str$ = prima$ + dopo$
        ELSE
        	ascii_str$ = prima$ + MID$(dopo$,1,k) + "." + REMAIN$(dopo$,MID$(dopo$,1,k))
    	END IF
    
    ELSEIF INSTR(ascii_str$,"E-") THEN
		k = VAL(REMAIN$(ascii_str$,"E-"))
    	ascii_str$ = EXTRACT$(ascii_str$,"E-")
    	'sposto il punto decimale di k posizioni indietro fillando eventualmente con zeri
    	prima$ = EXTRACT$(ascii_str$,".")
    	dopo$ = REMAIN$(ascii_str$,".")
        'print "k= ",k
        'print "prima$= ",prima$
        'print "dopo$= ",dopo$
    	IF INCHR(prima$,"-") = 0 THEN
    		IF LEN(prima$) < k THEN
    			'fillo con zeri prima$
    			FOR cnt = LEN(prima$) TO k-1
    				prima$ = "0" + prima$
    			NEXT cnt
                ascii_str$ = "." + prima$ + dopo$
    		ELSE
    			ascii_str$ = MID$(prima$,LEN(prima$)-k,k-1) + "." + REMAIN$(prima$,MID$(prima$,LEN(prima$)-k,k-1)) + dopo$
    		END IF
        ELSE
        prima$ = REMAIN$(prima$,"-")
    		IF LEN(prima$) < k THEN
    ...
    'ELSEIF INSTR(ascii_str$,"E0") THEN
    	'ascii_str$ = EXTRACT$(ascii_str$,"E0")
...
SUB Info
	...
	'quadexpformat(cx1,qcx1$,200,2)
	'quadexpformat(cy1,qcy1$,200,2)
	'quadexpformat(cx2,qcx2$,200,2)
	'quadexpformat(cy2,qcy2$,200,2)
