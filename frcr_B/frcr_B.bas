...
IF COMMAND$ = ""  OR COMMAND$(1) = "?" THEN
    PRINT "FrCr_b: Fractal Crypter by Stefano Trevisani (c) 2007 Ver 0.1"
    PRINT " Usage: FrCr_b infile outfile keyfile [option]"
    PRINT " [-c] Crypt"
    PRINT " [-d] DeCrypt"
    PRINT BEL$
    END
...
'faccio un casting a double
    LOCAL cforma AS DOUBLE
    cforma = quad2double(cf)

'trasformo in valore assoluto il coefficiente di forma
    IF cforma < 0.0 THEN cforma = -cforma

'ricavo le dimensioni in word del file
    LOCAL FWord AS UINT
    FWord = Round(Fsize/2.0,0)

'calcolo dimensioni window
'ymassimo window
    v = Round((FWord / cforma)^0.5,0)
'xmassimo window
    o = Round((FWord * cforma)^0.5,0)
...
SUB LeggiChiave
    ...
    global cx1 as pBignum
    global cy1 as pBignum
    global cx2 as pBignum
    global cy2 as pBignum
